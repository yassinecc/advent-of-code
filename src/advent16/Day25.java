...
public class Day25 extends tools{
 ...
 private static void toggle(List<String> inputFile, int i, Map<String,Integer> map, String[] elements) {
    ...
    Integer bit = 2;
		loop : while(i>=0 && i<inputFile.size()){
      ...
      case "add" :
        increment(map, elements, Integer.parseInt(elements[2]));

      case "out" :
        int oldBit = new Integer(bit);
        bit = map.get(elements[1]);
        if(bit==oldBit) break loop;
	...
	public static void main(String[] args) {
		List<String> inFile1 = readFile("input25.txt");
